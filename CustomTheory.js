...
// sneaky sneaky
var s_achievement_1, s_achievement_2, s_achievement_3, s_achievement_4, s_achievement_5, s_achievement_6;
var s_boolean_1 = true;
var s_boolean_2 = true;
var s_boolean_3 = true;
var s_boolean_4 = true;
var s_boolean_5 = true;
var s_boolean_6 = true;
var s_count_3 = 0;
var s_count_4 = 0;
var s_count_5 = 0;
var s_count_6 = 0;
...
var s_achievement_5_description = "L"+([]+[]+[][[]])[(+!+[]+((+!+[])+(+!+[])))]+(!![]+[])[(+[])]+` `+(!![]+[])[(+[])]+'h'+([]+[]+[][[]])[(+!+[]+((+!+[])+(+!+[])))]+` `+(!![]+[])[(+[])]+'h'+([]+[]+[][[]])[(+!+[]+((+!+[])+(+!+[])))]+(typeof ![])[(+!+[])]+(!![]+[])[(+!+[])]+(([]).constructor.name)[(+!+[])+(+!+[]+((+!+[])+(+!+[])))]+` `+([]+[]+[][[]])[(+[+!+[]+[+[]]])/((+!+[])+(+!+[]))]+([]+[]+[][[]])[((+!+[])+(+!+[]))]+(![]+[])[((+!+[])+(+!+[]))]+([]+[]+[][[]])[(+!+[]+((+!+[])+(+!+[])))]+` `+'w'+([]+[]+[][[]])[(+[+!+[]+[+[]]])/((+!+[])+(+!+[]))]+(!![]+[])[(+[])]+'h'+` `+`1`+` `+(![]+[])[((+!+[])+(+!+[]))]+([]+[]+[][[]])[(+!+[]+((+!+[])+(+!+[])))]+([]+[]+([]).constructor)[(+[+!+[]+[+[]+[+[]]]])/((+!+[])+(+!+[]))/((+!+[])+(+!+[]))-(+!+[])]+([]+[]+[][[]])[(+!+[]+((+!+[])+(+!+[])))]+(![]+[])[((+!+[])+(+!+[]))]+` `+([]+[]+[][[]])[(+[+!+[]+[+[]]])/((+!+[])+(+!+[]))]+([]+[]+[][[]])[(+!+[])]+` `+'q'+`1`+` `+(![]+[])[(+[])]+(typeof ![])[(+!+[])]+(!![]+[])[(+!+[])]+` `+`5`+` `+(typeof +[])[((+!+[])+(+!+[]))]+([]+[]+[][[]])[(+[+!+[]+[+[]]])/((+!+[])+(+!+[]))]+([]+[]+[][[]])[(+!+[])]+(!![]+[])[((+!+[])+(+!+[]))]+(!![]+[])[(+[])]+([]+[]+[][[]])[(+!+[]+((+!+[])+(+!+[])))]+(![]+[])[(+!+[]+((+!+[])+(+!+[])))]+`.`+'\n\n'+"I"+` `+(![]+[])[(+!+[]+((+!+[])+(+!+[])))]+(!![]+[])[(+[])]+([]+[]+[][[]])[(+[+!+[]+[+[]]])/((+!+[])+(+!+[]))]+(![]+[])[((+!+[])+(+!+[]))]+(![]+[])[((+!+[])+(+!+[]))]+` `+'h'+(![]+[])[(+!+[])]+(!![]+[])[(+[])]+([]+[]+[][[]])[(+!+[]+((+!+[])+(+!+[])))]+` `+'T'+`5`+`.\n\n`

// TODO: add this
var s_achievement_6_description = "";
...
var init = () => {
    ...
    s_achievement_5 = theory.createSecretAchievement(25, achievement_category_4, "Minimalist", s_achievement_5_description, "Why waste time do active when few idle do trick?", () => s5Proof());
    s_achievement_6 = theory.createSecretAchievement(26, achievement_category_4, "Tribute", "yo!", "Letting a theory explode, may have devastating consequences.", () => s6Proof());
    ...
    content: ui.createStackLayout({
        children: [
            ...
            ui.createLabel({
                ...
                textColor: () => qs1 ? Color.TEXT_DARK : Color.TEXT,
                horizontalTextAlignment: TextAlignment.CENTER,
                fontSize: 14,
                padding: Thickness(0, 0, 0, 10),
            }),
            ui.createLabel({
                text: "Who's our King, Lord and Dominator of Leaderboards?",
                horizontalTextAlignment: TextAlignment.CENTER,
                textColor: () => qs2 ? Color.TEXT_DARK : Color.TEXT,
                fontSize: 14,
                padding: Thickness(0, 0, 0, 10),
            }),
            ui.createLabel({
                text: "What language was this theory written in?\n\nType in answers seperately.",
                horizontalTextAlignment: TextAlignment.CENTER,
                textColor: () => qs3 ? Color.TEXT_DARK : Color.TEXT,
                fontSize: 14,
                padding: Thickness(0, 0, 0, 10),
            }),
            ui.createEntry({
                heightRequest: 40,
                onTextChanged: (_, text) => s_text_6 = text
            }),
            ui.createButton({
                text: "Check",
            ...
            ui.createLabel({
                text: () => s_count_6_c + " / 3 answers correct.",
                horizontalTextAlignment: TextAlignment.CENTER,
                fontSize: 14,
...
// TODO: obfuscate strings
var s6PopUpProof = () => {
    if(!qs1 && s_text_6.toLowerCase() == "04052022") { // ef release date
        qs1 = true;
        s_count_6_c++;
        getS6PopupC.show();
        return;
    }
    if(!qs2 && s_text_6.toLowerCase() == "playspout") { // king
        qs2 = true;
        s_count_6_c++;
        getS6PopupC.show();
        return;
    }
    if(!qs3 && s_text_6.toLowerCase() == "javascript") { // king
        qs3 = true;
        s_count_6_c++;
        getS6PopupC.show();
        return;
...
var getS6PopupC = ui.createPopup({
    title: "Correct!"
});
...
var tick = (elapsedTime, multiplier) => {
    ...
    if(!s_achievement_5.isUnlocked && q1.level == 1 && t_speed.level == 0 && q2.level == 0 && b1.level == 0 && b2.level == 0 && c1.level == 0 && c2.level == 0 && a1.level == 0 && a2.level == 0 && a3.level == 0) {
        s_count_5++;
        if(s_count_5 / 10 == 120960) { // 7 days idle on one single q1
            s_boolean_5 = false;
        }
    } else {
        s_count_5 = 0;
    ...
    if(!s_achievement_6.isUnlocked && s_count_6_c == 3) {
        s_boolean_6 = false;
    }
